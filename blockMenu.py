    ...
    def handleEvents(self, events: list[pygame.event.Event]):
        for event in events:
            if event.type == pygame.MOUSEWHEEL and self.isTouchingMouse():
                if (
                    event.y > 0
                    or (len(self.items) - 1 - self.scrollPos) * (self.iconSize + 20)
                    + self.iconSize
                    + 50
                    > self.size.y
                ):
                ...
                if self.scrollPos < 0 or len(self.items) == 0:
                    self.scrollPos = 0
                if (len(self.items) - 1 - self.scrollPos) * (
                    self.iconSize + 20
                ) + self.iconSize + 50 < self.size.y:
                    fixPos = -(
                        (self.size.y - self.iconSize - 50) / (self.iconSize + 20)
                        - len(self.items)
                        + 1
                    )
                    if fixPos > 0:
                ...
                if event.button == 1 and self.isTouchingMouse():
                    ...
                    while i - 10 < self.scrollPos:
                        ...
                        icon = pygame.transform.scale_by(
                            icon, self.iconSize / max(icon.get_size()[0], icon.get_size()[1])
                        )
                        ...
                        if Helpers.isSurfAtPosTouchingPos(
                            ...
                            icon,
                            self.screenPosToSurfPos(Helpers.getMousePos()),
                        ):
    ...
    def draw(self):
        ...
        while i - 10 < self.scrollPos:
            ...
            if item == self.selectedItem:
                ...
                glowPos = (
                    pos[0] + (icon.get_size()[0] - glow.get_size()[0]) / 2,
